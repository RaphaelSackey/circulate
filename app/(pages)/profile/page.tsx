...
export default function Profile() {
	...
	// logic to check if the user is logged it before accessing the page
	const router = useRouter();
	const {
		data: signInData,
		isPending: userLoggedInPending,
		isSuccess: userLoggedInSuccess,
	} = useSignedIn();

	useLayoutEffect(() => {
		if (!userLoggedInPending && !userLoggedInSuccess) {
			router.push("/");
		}
	}, [userLoggedInSuccess, userLoggedInPending]);
			...
			<div className='container mx-auto my-10 px-4 h-[100vh]'>
				<div className='lg:grid grid-cols-4 gap-6 rounded-md'>
					...
					<div className='hidden lg:flex flex-col justify-center items-center border-2 m-6 rounded-lg'>
						...
						<h2 className='text-4xl font-semibold'>{UserItemsData?.trustScore}</h2>
					</div>
				</div>
